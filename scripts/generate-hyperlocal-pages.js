...
function buildCityZipHub(state, cityZip, nearbyZips) {
  ...
  const title = `Plumbers in ${cityName}, ${stateCode} ${zip} | 24/7 Plumbing`;

  // Unique Meta Description (Strictly 150-160 chars)
  const metaDesc = `Looking for trusted plumbers in ${cityName}, ${stateCode} (${zip})? Home Plumbing USA connects you with vetted, 24/7 licensed local experts for repairs. Call 877-516-8705!`;

  const servicesGridHtml = SERVICES.map(s => `
    <div class="service-card animate-on-scroll" style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 28px; transition: var(--transition);">
      <div class="service-icon" style="width: 50px; height: 50px; border-radius: 12px; background: var(--gradient-primary); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 1.3rem; margin-bottom: 18px;"><i class="fas fa-wrench"></i></div>
      <h3 style="font-size: 1.3rem; font-weight: 800; color: var(--text-white); margin-bottom: 10px;">${s.name}</h3>
      <p style="color: var(--text-muted); font-size: 0.95rem; line-height: 1.6; margin-bottom: 16px;">Professional ${s.name.toLowerCase()} for residential and commercial properties in ${cityName} (${zip}).</p>
      <a href="/${stateSlug}/${cityZipSlug}/${s.slug}/" style="color: var(--primary); font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; font-size: 0.95rem;">View ${s.name} <i class="fas fa-arrow-right" style="font-size: 0.8rem;"></i></a>
    </div>
  `).join('\n');

  const nearbyHtml = nearbyZips.slice(0, 8).map(nz => `
    <a href="/${stateSlug}/${nz.slug}/" class="area-card" style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: var(--radius-sm); padding: 16px 20px; display: flex; align-items: center; justify-content: space-between; text-decoration: none; color: var(--text-white); font-weight: 600;">
      <span><i class="fas fa-map-pin" style="color: var(--primary); margin-right: 8px;"></i> ${nz.city} (${nz.zip})</span>
...
<head>
  ...
  <title>${title}</title>
  <meta name="description" content="${metaDesc}">
  <meta name="keywords" content="plumbers ${cityName} ${zip}, emergency plumbing ${cityName}, 24 hour plumber ${cityName} ${stateCode}, local plumbers ${zip}">
  <link rel="canonical" href="${pageUrl}">
...
<body data-prefix="/" data-depth="0">
  ...
  <header class="header" id="header" style="min-height: 120px;">
    <div class="top-bar" style="min-height: 40px; height: 40px; display: flex; align-items: center; justify-content: center; text-align: center; white-space: nowrap;">
      <div class="top-bar-content">
        <span class="pulse-dot"></span>
        <span>24/7 Emergency Plumbers in <strong>${cityName} (${zip})</strong> - Licensed & Certified!</span>
      </div>
    </div>
    <div class="header-inner" style="min-height: 80px; height: 80px; display: flex; align-items: center; justify-content: space-between;">
      <a href="/" class="logo" style="width: 247px; max-width: 100%; display: flex; align-items: center;">
      ...
      <nav class="nav" id="mainNav">
        <a href="/" class="nav-link">Home</a>
        <a href="${stateUrl}" class="nav-link">${stateName}</a>
        <a href="#services" class="nav-link">Services in ${zip}</a>
        <a href="/contact" class="nav-link">Contact</a>
      </nav>
      <div class="header-cta">
        <a href="tel:877-516-8705" class="header-phone"><i class="fas fa-phone"></i> 877-516-8705</a>
  ...
  <main>
    <!-- HERO -->
    <section class="hero" style="padding: 70px 0 50px; background: var(--gradient-hero);">
      <div class="container">
        <div style="display: flex; gap: 8px; align-items: center; font-size: 0.9rem; margin-bottom: 20px; color: var(--text-muted);">
          <a href="/" style="color: var(--primary); text-decoration: none;">Home</a>
          <i class="fas fa-chevron-right" style="font-size: 0.75rem;"></i>
          <a href="${stateUrl}" style="color: var(--primary); text-decoration: none;">${stateName}</a>
          <i class="fas fa-chevron-right" style="font-size: 0.75rem;"></i>
        ...
        <div class="hero-grid">
          <div class="hero-content">
            <div class="hero-badge"><i class="fas fa-star"></i> Vetted Local Plumbing Specialists in ${cityName}</div>
            <h1 style="font-size: 3rem; font-weight: 900; line-height: 1.15; color: #fff; margin: 12px 0 18px;">Licensed Plumbers in <span>${cityName}, ${stateCode} (${zip})</span></h1>
            <p class="hero-text" style="font-size: 1.15rem; color: rgba(255,255,255,0.9); line-height: 1.6; margin-bottom: 24px;">Connecting homeowners and commercial businesses in ${cityName} (${zip}) with licensed, background-checked plumbers 24/7. From emergency leak and burst pipe repairs to high-pressure drain jetting and water heater replacements, get immediate dispatch in ${cityName}.</p>
            <div class="hero-buttons">
              <a href="tel:877-516-8705" class="btn btn-accent btn-lg"><i class="fas fa-phone"></i> Call 877-516-8705</a>
              <a href="#services" class="btn btn-outline btn-lg"><i class="fas fa-wrench"></i> View Services in ${zip}</a>
            </div>
          </div>
          <div class="hero-image-wrapper">
            <div class="hero-image-container" style="max-width: 420px; aspect-ratio: 4 / 5; background: #0f1e3a; border-radius: var(--radius-md); overflow: hidden; border: 1px solid var(--border-color);">
              <img src="/public/images/hero-plumbing.webp" srcset="/public/images/hero-plumbing-mobile.webp 480w, /public/images/hero-plumbing.webp 1200w" sizes="(max-width: 600px) 480px, 1200px" alt="Plumbing Services in ${cityName}, ${stateCode}" class="hero-image" width="600" height="750" fetchpriority="high" style="width: 100%; height: 100%; object-fit: cover;">
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="section" id="services" style="background: var(--bg-surface);">
      <div class="container">
        <div class="section-header" style="text-align: center; margin-bottom: 40px;">
          <div class="section-label" style="display: inline-block; padding: 4px 14px; background: rgba(37,99,235,0.15); color: var(--primary); border-radius: 20px; font-weight: 700; font-size: 0.85rem; text-transform: uppercase; margin-bottom: 10px;">Capabilities</div>
          <h2 style="font-size: 2.2rem; font-weight: 800; color: #fff;">Plumbing Services in ${cityName} (${zip})</h2>
          <p style="color: var(--text-muted); font-size: 1.05rem; max-width: 700px; margin: 10px auto 0;">Select your required plumbing service below for diagnostic protocols, upfront pricing, and fast dispatch in ${cityName} (${zip}).</p>
        </div>
    ...
    <section class="section">
      <div class="container">
        <div class="section-header" style="text-align: center; margin-bottom: 30px;">
          <h2 style="font-size: 1.8rem; font-weight: 800; color: #fff;">Other ${stateName} Communities We Serve</h2>
        </div>
    ...
    <div class="container">
      <div class="footer-grid">
        <div class="footer-about">
          ...
          <h3 class="footer-title">24/7 Plumbers in ${cityName}, ${stateCode}</h3>
          <p class="footer-brand-subheading">Serving ${cityName} (${zip}) and surrounding areas with licensed local plumbing experts.</p>
        </div>
        <div class="footer-col">
          <div class="footer-title">Quick Links</div>
          <a href="/">Home</a>
        ...
        <div class="footer-col">
          <div class="footer-title">24/7 Dispatch</div>
          <p>Direct line to active dispatch:</p>
          <a href="tel:877-516-8705" class="footer-call-btn" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 24px; background: var(--accent); color: #fff; border-radius: 8px; text-decoration: none; font-weight: 700;"><i class="fas fa-phone"></i> 877-516-8705</a>
        </div>
      </div>
      <div class="footer-bottom">
