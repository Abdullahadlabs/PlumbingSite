...
const FLORIDA_DIR = path.join(__dirname, '..', 'florida');
...
function main() {
  console.log('=== Verifying Florida Service Pages ===');

  const args = process.argv.slice(2);
  const sampleLimitArg = args.find(a => a.startsWith('--samples='))?.split('=')[1];
  const checkAll = sampleLimitArg === 'all';
  const sampleLimit = checkAll ? 999999 : (sampleLimitArg ? parseInt(sampleLimitArg, 10) : 100);

  const seoData = JSON.parse(fs.readFileSync(seoPagesPath, 'utf8'));
  const flLocations = seoData.filter(d => d.state === 'FL');

  console.log(`Loaded ${flLocations.length} Florida locations.`);
  console.log(`Checking ${checkAll ? 'ALL ' + flLocations.length : sampleLimit} locations across all 8 services...`);

  let testLocations = [];
  if (checkAll || sampleLimit >= flLocations.length) {
    testLocations = flLocations;
  } else {
    const step = Math.max(1, Math.floor(flLocations.length / sampleLimit));
    for (let i = 0; i < flLocations.length && testLocations.length < sampleLimit; i += step) {
      testLocations.push(flLocations[i]);
    }
  }
  ...
  testLocations.forEach(loc => {
    ...
    SERVICES.forEach(serviceSlug => {
      ...
      // 2. Verify Key Structural Reference Sections
      ...
      for (const sec of requiredSections) {
        if (!content.includes(sec)) {
          issues.push(`Missing section/text "${sec}" on ${cityZipSlug}/${serviceSlug}`);
          failedCount++;
          return;
        }
      }
      ...
      // 4. Verify no leaks of other states/cities
      const leaks = ['Lakewood', '80226', 'Colorado State Plumbing Board', 'Anchorage', '99507', 'Texas State Board', 'Houston, TX'];
  ...
  if (issues.length > 0) {
    ...
    console.log('\nAll checked Florida service pages passed verification with 0 errors and 0 state leaks!');
  }
}
